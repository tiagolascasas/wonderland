# 1 "VitisClavaAutomation/src/example.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 359 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "VitisClavaAutomation/src/example.c" 2


void maddc(int B[4096], int A[4096], int C) {
 VITIS_LOOP_4_1: for (int i = 0; i < 4096; i++) {
#pragma HLS unroll factor = 32
 B[i] = A[i] + C;
 }
}

__attribute__((sdx_kernel("maddv", 0))) void maddv(int C[4096], int A[4096], int B[4096]) {
#line 14 "C:/Users/Tiago/Dev/ClavaExperiments/script.tcl"
#pragma HLSDIRECTIVE TOP name=maddv
# 10 "VitisClavaAutomation/src/example.c"

 VITIS_LOOP_11_1: for (int i = 0; i < 4096; i++) {
#pragma HLS unroll factor = 32
 C[i] = A[i] + B[i];
 }
}
