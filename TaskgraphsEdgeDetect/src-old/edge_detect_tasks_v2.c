...
void edge_detect(int image_rgb[H][W * 3], int image_gray[H][W], int temp_buf[H][W], int filter[K][K], int filter_temp[K][K], int output[H][W])
{
...
int main()
{
    ...
    {
#endif
        edge_detect(image_rgb, image_gray, temp_buf, filter, filter_temp, output);
        if (i == 0)
        {
            checksum(output);
        }
