...
#pragma oss task device(smp) in([1] mapped_epoch)
void print_task(int *mapped_epoch)
{
    printf("Epoch %d done\n", *mapped_epoch);
}
...
#pragma oss task device(fpga) in([4608000] data, [4500] label)inout([1024] theta, [1] mapped_epoch)
...
int main(int argc, char *argv[])
{
    ...
    int *mapped_epoch = (int *)malloc(sizeof(int));

    SgdLR_sw(data_points, labels, param_vector, mapped_epoch);
#pragma oss taskwait
