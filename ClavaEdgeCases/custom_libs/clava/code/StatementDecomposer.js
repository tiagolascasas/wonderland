...
class StatementDecomposer {
    ...
    #isValidVardecl($vardecl) {
        const condition = $vardecl.hasInit && $vardecl.type.isAuto;
        println($vardecl.hasInit + ", " + $vardecl.children.length + ", " + $vardecl.type.isAuto + ", " + $vardecl.code);
        return !condition;
    }
    ...
    #decomposeDecl($decl) {
        ...
        if (!this.#isValidVardecl($decl)) {
            debug(`StatementDecomposer.decomposeDeclStmt: invalid vardecl "${$decl.code}", skipping`);
            return [ClavaJoinPoints.declStmt($decl)];
        }
